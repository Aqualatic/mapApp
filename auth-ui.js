...
//
// NOTE: Markers are ONLY loaded/saved when the user clicks the
// buttons. No automatic loading on sign-in or page load.
// ============================================================
...
(function initAuthUI() {
  ...
  style.textContent = `
    ...
    .auth-modal-content {
      ...
      position: relative; box-shadow: 0 8px 32px rgba(0,0,0,0.6); color: #fff;
    }
    .auth-modal-close {
    ...
    .auth-label { display: block; font-size: 12px; color: #aaa; margin-bottom: 4px; margin-top: 12px; }
    .auth-input {
    ...
    .auth-error {
      ...
      font-size: 13px; color: #ffaaaa; margin-top: 12px;
    }
    .auth-submit {
  ...
  // ----------------------------------------------------------
  // Supabase auth — UI updates only, NO auto-load
  // ----------------------------------------------------------
  ...
  // ----------------------------------------------------------
  // Save — only triggered by button click
  // ----------------------------------------------------------
  authSaveDataBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Auth UI] Save failed:', err);
    } finally {
  ...
  // ----------------------------------------------------------
  // Load — only triggered by button click
  // ----------------------------------------------------------
  authLoadDataBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      console.error('[Auth UI] Load failed:', err);
    } finally {
