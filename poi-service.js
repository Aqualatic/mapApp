 ...
 * Increase limit to find the nearest POI, not just any nearby result.
 */
async function reverseSearchPOI(lat, lng, accessToken) {
  const url = `${POI_SEARCH_BOX_BASE}/reverse?longitude=${lng}&latitude=${lat}&limit=5&types=poi&language=en&access_token=${accessToken}`;
 ...
 * Uses a tight bounding box (~200m) to constrain results.
 */
async function categorySearchPOI(category, lat, lng, accessToken) {
  // ~200m bounding box (0.002 degrees ≈ 220m at equator)
  const offset = 0.002;
  const bbox = `${lng - offset},${lat - offset},${lng + offset},${lat + offset}`;
  const url = `${POI_SEARCH_BOX_BASE}/category/${category}?proximity=${lng},${lat}&bbox=${bbox}&limit=3&language=en&access_token=${accessToken}`;
  const res = await fetch(url);
...
async function fetchPOINearby(lat, lng, accessToken) {
  ...
  try {
    // Step 1: Reverse lookup (returns multiple, pick closest)
    const reverseData = await reverseSearchPOI(lat, lng, accessToken);
    if (reverseData.features && reverseData.features.length > 0) {
      // Find the closest POI that is actually near the click point
      for (const feature of reverseData.features) {
        if (feature.properties.feature_type === 'poi') {
          const coords = feature.geometry?.coordinates;
          if (coords) {
            const dist = haversineDistance(lat, lng, coords[1], coords[0]);
            if (dist < 150) { // Only accept POIs within 150m
    ...
    // Step 2: Parallel category searches with tight bounding box
    ...
    // Find the closest POI from all category results
    let bestFeature = null;
    let bestDistance = Infinity;

    for (const result of categoryResults) {
      if (!result || !result.features) continue;
    ...
    // Only use the category result if it's within ~200m of the click
    if (bestFeature && bestDistance < 200) {
    ...
    <div class="poi-popup-container">
      ...
      <div class="poi-save-section">
        <div class="poi-save-row">
          <input type="text" class="poi-category-input" id="${popupId}_cat" placeholder="Category tag" value="default">
          <button class="poi-save-btn" onclick="window.savePOIAsMarker(${clickedLat}, ${clickedLng}, '${escapeHtmlPOI(name).replace(/'/g, "\\'").replace(/"/g, '&quot;')}', document.getElementById('${popupId}_cat').value, decodeURIComponent('${featureJSON}'))">
            <span class="poi-save-icon">📌</span> Save
          </button>
        </div>
      </div>
    </div>
  ...
  return `
    <div class="poi-popup-container poi-fallback">
      ...
      <div class="poi-save-section">
        <div class="poi-save-row">
          <input type="text" class="poi-category-input" id="${popupId}_cat" placeholder="Category tag" value="default">
          <button class="poi-save-btn" onclick="window.savePOIAsMarker(${lat}, ${lng}, 'Pinned Location', document.getElementById('${popupId}_cat').value, null)">
            <span class="poi-save-icon">📌</span> Save
          </button>
        </div>
      </div>
